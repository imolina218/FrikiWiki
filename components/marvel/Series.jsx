...
const Series = ({publicKey, hash}) => {
    ...
    let api = `https://gateway.marvel.com/v1/public/series?ts=1&apikey=${publicKey}&hash=${hash}&limit=50&offset=${pageNumber}`;
    ...
    if(modal) {
        let { title, description, comics, thumbnail, creators, events, stories, characters } = info.result;
        return (
                ...
                <div className={styles.contentimg}>
                    ...
                    <div className={styles.info}>
                        ...
                        {infoType !== false &&
                            ...
                            infoType === "series" ? <div>{events.items.map(event => ( <p key={event.resourceURI}>{event.name}</p> ))}</div> :
